...
const BotMoveHandler = () => {
  ...
  useEffect(() => {
    const performBotMove = async () => {
      await makeBotMove(gameState, botPlayer, difficulty, isPlayWithBot, isXNext, winner, actions);
    };
    
    if (isPlayWithBot && !winner) {
      performBotMove();
    }
  }, [gameState, winner, botPlayer, isXNext, isPlayWithBot, isCenterAvailable, actions])
